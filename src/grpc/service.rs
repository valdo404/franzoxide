...
impl ConnectorService for ConnectorServiceImpl {
                    ...
                    Ok(req) => {
                        ...
                        match req.request {
                            Some(source_request::Request::Heartbeat(heartbeat)) => {
                                log::debug!("Received heartbeat from source connector: {:?}", heartbeat);
                                
                                // Send a heartbeat response
                                let resp = SourceResponse {
                                    response: Some(source_response::Response::Heartbeat(Heartbeat {
                    ...
                    Ok(req) => {
                        ...
                        match req.request {
                            Some(sink_request::Request::Heartbeat(heartbeat)) => {
                                log::debug!("Received heartbeat from sink connector: {:?}", heartbeat);
                            ...
                            Some(sink_request::Request::RecordBatch(batch)) => {
                                log::debug!("Received record batch from sink connector with {} records", batch.records.len());
                                
                                // Process the record batch
                                // In a real implementation, this would forward the records to the sink connector
                                
                                // Send an acknowledgment
                                let record_ids = batch.records.iter().map(|record| {
                                    RecordId {
                            ...
                            Some(sink_request::Request::Flush(flush)) => {
                                log::debug!("Received flush request from sink connector: {:?}", flush);
                                ...
                                let resp = SinkResponse {
                                    response: Some(sink_response::Response::FlushResponse(FlushResponse {
    ...
    async fn get_config(
        ...
        let connector_config = self.config.connectors.iter()
            .find(|c| c.name == req.connector_name)
            .ok_or_else(|| Status::not_found(format!("Connector not found: {}", req.connector_name)))?;
        ...
        Ok(Response::new(ConfigResponse { config: Some(config) }))
    }
    ...
    async fn update_config(
        ...
        let req = request.into_inner();
        let config = req.config.ok_or_else(|| Status::invalid_argument("Missing connector configuration"))?;
    ...
    async fn get_status(
        ...
        let status = StatusResponse {
            ...
            tasks: (0..connector_config.tasks_max).map(|i| {
                TaskStatus {
                    task_id: i,
                    state: status_response::State::Running as i32,
