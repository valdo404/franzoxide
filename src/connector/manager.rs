...
impl ConnectorManager {
    ...
    async fn initialize_connector(&mut self, connector_config: &ConnectorConfig) -> ConnectorResult<()> {
    ...
    async fn initialize_source_connector(&mut self, connector_config: &ConnectorConfig) -> ConnectorResult<()> {
        let name = connector_config.name.clone();
        let connector_class = connector_config.connector_class.clone();
        
        // Create task configs
        let tasks = (0..connector_config.tasks_max).map(|i| {
            TaskConfig {
                task_id: i,
        ...
        for task_config in tasks {
            ...
            // Store the connector
            self.source_connectors.insert(task_name, Arc::new(Mutex::new(source_connector)));
        }
    ...
    async fn initialize_sink_connector(&mut self, connector_config: &ConnectorConfig) -> ConnectorResult<()> {
        let name = connector_config.name.clone();
        let connector_class = connector_config.connector_class.clone();
        ...
        for task_config in tasks {
            ...
            let sink_connector: Box<dyn SinkConnector + Send> = match connector_class.as_str() {
                ...
                _ => {
                    return Err(ConnectorError::ConfigError(format!(
                        "Unsupported sink connector class: {}", connector_class
                    )));
                }
            };
            
            // Store the connector
            self.sink_connectors.insert(task_name, Arc::new(Mutex::new(sink_connector)));
        }
    ...
    async fn create_sink_for_source(
        ...
        if self.sink_connectors.is_empty() {
            return Err(ConnectorError::ConfigError("No sink connectors available".to_string()));
        }
        ...
        tokio::spawn(async move {
            while let Some(records) = rx.recv().await {
                debug!("Forwarding {} records from {} to {}", records.len(), source_name, sink_name);
