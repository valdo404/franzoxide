...
async fn main() -> anyhow::Result<()> {
    ...
    let config_path = args.get(1).map(|s| s.as_str()).unwrap_or("config/connect.json");
    ...
    if let Some(tcp_addr) = config.tcp_address {
        ...
        let tcp_server = Server::builder()
            .add_service(kafka_connect::connector_service_server::ConnectorServiceServer::new(connector_service.clone()))
            .serve(addr);
    ...
    if let Some(unix_socket_path) = config.unix_socket_path {
        ...
        let uds_server = tonic::transport::Server::builder()
            .add_service(kafka_connect::connector_service_server::ConnectorServiceServer::new(connector_service))
